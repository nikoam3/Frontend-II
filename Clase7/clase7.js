...
function mostrarDatosEnPerfil(listado) {
  ...
  let cantidad_albunes = listado.length
  let cantidad_favoritos = 0
  
  const num_albumes = document.querySelector(".perfil #cant-albums")
  const num_fav = document.querySelector(".perfil #cant-favoritos")

  listado.forEach((lista) => {
    lista.like ? cantidad_favoritos++ : ""
  })
  num_albumes.innerText = cantidad_albunes === 1 ? `${cantidad_albunes} álbum` : `${cantidad_albunes} álbumes`
  num_fav.innerText = cantidad_favoritos === 1 ? `${cantidad_favoritos} favorito` : `${cantidad_favoritos} favoritos`  
}
